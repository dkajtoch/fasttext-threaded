...
class ElapsedTimeMarker {
 ...
 public:
  ElapsedTimeMarker() { start_ = std::chrono::steady_clock::now(); }
  double getElapsed() {
...
template <typename T>
T getArgGauss(T val, std::minstd_rand& rng, double startSigma, double endSigma,
              double t, bool linear) {
  T returnValue;
  const double stddev = startSigma - ((startSigma - endSigma) / 0.5) *
...
template <typename T>
T updateArgGauss(T val, T min, T max, double startSigma, double endSigma, double t,
                 bool linear, std::minstd_rand& rng) {
...
AutotuneStrategy::AutotuneStrategy(const Args& originalArgs,
...
Args AutotuneStrategy::ask(double elapsed) {
  ...
  if (!args.isManual("minn")) {
    int minnIndex =
        updateArgGauss(bestMinnIndex_, 0, static_cast<int>(minnChoices_.size() - 1),
                       4.0, 1.4, t, true, rng_);
  ...
  if (!args.isManual("bucket")) {
    int nonZeroBucket =
        updateArgGauss(bestNonzeroBucket_, 10000, 10000000, 2.0, 1.5, t, false, rng_);
...
void Autotune::timer(const std::chrono::steady_clock::time_point& start,
                     double maxDuration) {
  elapsed_ = 0.0;
...
double Autotune::getMetricScore(Meter& meter, const metric_name& metricName,
...
int Autotune::getCutoffForFileSize(bool qout, bool qnorm, int dsub,
...
bool Autotune::quantize(Args& args, const Args& autotuneArgs) {
  ...
  args.retrain = true;
  args.cutoff = getCutoffForFileSize(args.qout, args.qnorm, args.dsub,
                                     autotuneArgs.getAutotuneModelSize());
...
void Autotune::printSkippedArgs(const Args& autotuneArgs) {
  std::unordered_set<std::string> argsToCheck = {
      "epoch", "lr", "dim", "wordNgrams", "loss", "bucket", "minn", "maxn", "dsub"};
  ...
  while (keepTraining(autotuneArgs.autotuneDuration)) {
    ...
    try {
      ...
      if (sizeConstraintOK) {
        ...
        fastText_->test(validationFileStream, autotuneArgs.autotunePredictions, 0.0,
                        meter);

        currentScore =
            getMetricScore(meter, autotuneArgs.getAutotuneMetric(),
