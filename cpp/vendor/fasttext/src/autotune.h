...
class AutotuneStrategy {
 ...
 public:
  explicit AutotuneStrategy(const Args& args, std::minstd_rand::result_type seed);
...
class Autotune {
 protected:
  ...
  void timer(const std::chrono::steady_clock::time_point& start, double maxDuration);
  void abort();
  void startTimer(const Args& args);
  double getMetricScore(Meter& meter, const metric_name& metricName,
                        const double metricValue, const std::string& metricLabel) const;
  ...
  int getCutoffForFileSize(bool qout, bool qnorm, int dsub, int64_t fileSize) const;
