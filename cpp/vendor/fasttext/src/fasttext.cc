...
std::shared_ptr<Loss> FastText::createLoss(std::shared_ptr<Matrix>& output) {
  ...
  switch (lossName) {
    ...
    case loss_name::ns:
      return std::make_shared<NegativeSamplingLoss>(output, args_->neg,
                                                    getTargetCounts());
...
std::shared_ptr<const Dictionary> FastText::getDictionary() const { return dict_; }
...
void FastText::saveOutput(const std::string& filename) {
  ...
  for (int32_t i = 0; i < n; i++) {
    std::string word =
        (args_->model == model_name::sup) ? dict_->getLabel(i) : dict_->getWord(i);
    vec.zero();
...
void FastText::quantize(const Args& qargs, const TrainCallback& callback) {
  ...
  input_ =
      std::make_shared<QuantMatrix>(std::move(*(input.get())), qargs.dsub, qargs.qnorm);
...
void FastText::supervised(Model::State& state, real lr,
...
void FastText::skipgram(Model::State& state, real lr,
                        const std::vector<int32_t>& line) {
  std::uniform_int_distribution<> uniform(1, args_->ws);
  for (int32_t w = 0; w < line.size(); w++) {
    int32_t boundary = uniform(state.rng);
...
std::tuple<int64_t, double, double> FastText::test(std::istream& in, int32_t k,
  ...
  return std::tuple<int64_t, double, double>(meter.nexamples(), meter.precision(),
                                             meter.recall());
}
...
void FastText::lazyComputeWordVectors() {
  if (!wordVectors_) {
    wordVectors_ =
        std::unique_ptr<DenseMatrix>(new DenseMatrix(dict_->nwords(), args_->dim));
    precomputeWordVectors(*wordVectors_);
  }
}

std::vector<std::pair<real, std::string>> FastText::getNN(const std::string& word,
                                                          int32_t k) {
...
void FastText::trainThread(int32_t threadId, const TrainCallback& callback) {
  ...
  if (threadId == 0) loss_ = state.getLoss();
  ifs.close();
}
...
std::shared_ptr<Matrix> FastText::getInputMatrixFromFile(
  ...
  in >> n >> dim;
  if (dim != args_->dim) {
    throw std::invalid_argument("Dimension of pretrained vectors (" +
                                std::to_string(dim) + ") does not match dimension (" +
                                std::to_string(args_->dim) + ")!");
  }
  ...
  std::shared_ptr<DenseMatrix> input =
      std::make_shared<DenseMatrix>(dict_->nwords() + args_->bucket, args_->dim);
