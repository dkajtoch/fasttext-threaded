...
class FastText {
 ...
 protected:
  ...
  std::vector<std::pair<real, std::string>> getNN(const DenseMatrix& wordVectors,
                                                  const Vector& queryVec, int32_t k,
  ...
  void supervised(Model::State& state, real lr, const std::vector<int32_t>& line,
 ...
 public:
  ...
  void setMatrices(const std::shared_ptr<DenseMatrix>& inputMatrix,
  ...
  std::tuple<int64_t, double, double> test(std::istream& in, int32_t k,
                                           real threshold = 0.0);

  void test(std::istream& in, int32_t k, real threshold, Meter& meter) const;

  void predict(int32_t k, const std::vector<int32_t>& words, Predictions& predictions,
               real threshold = 0.0) const;

  bool predictLine(std::istream& in,
                   std::vector<std::pair<real, std::string>>& predictions, int32_t k,
  ...
  std::vector<std::pair<real, std::string>> getAnalogies(int32_t k,
