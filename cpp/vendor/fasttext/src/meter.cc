...
void Meter::log(const std::vector<int32_t>& labels, const Predictions& predictions) {
  nexamples_++;
...
double Meter::precision(int32_t i) { return labelMetrics_[i].precision(); }

double Meter::recall(int32_t i) { return labelMetrics_[i].recall(); }

double Meter::f1Score(int32_t i) { return labelMetrics_[i].f1Score(); }

double Meter::precision() const { return metrics_.precision(); }

double Meter::recall() const { return metrics_.recall(); }
...
double Meter::precisionAtRecall(int32_t labelId, double recallQuery) const {
  ...
  std::for_each(
      precisionRecall.begin(), precisionRecall.end(),
...
std::vector<std::pair<double, double>> Meter::precisionRecallCurve(
  ...
  auto fullRecall = std::lower_bound(positiveCounts.begin(), positiveCounts.end(),
                                     golds, utils::compareFirstLess);
