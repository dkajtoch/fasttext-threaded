...
class Meter {
  struct Metrics {
    ...
    std::vector<std::pair<real, real>> getScoreVsTrue() { return scoreVsTrue; }
  };
  std::vector<std::pair<uint64_t, uint64_t>> getPositiveCounts(int32_t labelId) const;
 ...
 public:
  ...
  std::vector<std::pair<double, double>> precisionRecallCurve(int32_t labelId) const;
  std::vector<std::pair<double, double>> precisionRecallCurve() const;
  double precision() const;
  double recall() const;
  double f1Score() const;
  uint64_t nexamples() const { return nexamples_; }
